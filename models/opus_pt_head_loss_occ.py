...
'''
   add occupancy loss for lidar feature
'''

@HEADS.register_module()
class OPUS_PT_Head_loss_occupancy(BaseModule):
    def __init__(self,
                 num_classes,
                 in_channels,
                 num_query,
                 ...
                 loss_lidar_occ=dict(type='CrossEntropyLoss', use_sigmoid=True, loss_weight=1.0),
                 embed_dims=256,
                 init_cfg=None,
                 **kwargs):
        ...
        self.loss_lidar_occ = build_loss(loss_lidar_occ)
    ...
    def forward(self,
        ...
        '''
        Args:
            pts_feat:B,C,Dz,Dy,Dx
        '''
        init_points, query_feat = self.get_init_position(points, mlvl_feats,
                                                         pts_feats, img_metas)

        pts_feat_tmp=pts_feats.permute(0,4,3,2,1)
        occ_lidar=self.predicter(pts_feat_tmp)
    ...
    def get_lidar_occ_loss(self, occ_lidar, voxel_semantics):
        """
        Args:
            occ_lidar: (B, Dx, Dy, Dz,2)
            voxel_semantics: (B, Dx, Dy, Dz)
        """
        occ=(voxel_semantics!=self.empty_label).to(torch.int64)
        occ=occ.reshape(-1)
        occ_lidar=occ_lidar.reshape(-1,2)
        lidar_occ_loss=self.loss_lidar_occ(occ_lidar, occ)
        return lidar_occ_loss
    ...
    def loss(self, voxel_semantics, mask_camera, preds_dicts):
        ...
        occ_lidar = preds_dicts['occ_lidar']
        ...
        loss_lidar_occ=self.get_lidar_occ_loss(occ_lidar,voxel_semantics)
        loss_dict['loss_lidar_occ']=loss_lidar_occ
