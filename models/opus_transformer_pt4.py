...
'''
    adding feature:
        ...
        the fusion way:add+proj
'''

@TRANSFORMER.register_module()
class OPUSTransformer_PT4(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=6,
        ...
        self.decoder = OPUSTransformerDecoder_PT2(
...
class OPUSTransformerDecoder_PT2(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=6,
        ...
        for i in range(num_layers):
            self.decoder_layers.append(
                OPUSTransformerDecoderLayer_PT2(
...
class OPUSTransformerDecoderLayer_PT2(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=6,
    ...
    def forward(self, query_points, query_feat, mlvl_feats, pts_feats,occ2img, img_metas):
        ...
        ## B,Q,C
        img_sample_feat=self.mixing(sampled_feat, query_feat)
        sampling_pts=encode_points(sampling_pts, self.pc_range)
        sampling_pts=sampling_pts * 2 - 1 # B,Q,G,P,3

        # B,C,Q,G,P
        pts_sample_feat=F.grid_sample(pts_feats, sampling_pts, mode='nearest',padding_mode='zeros', align_corners=True) 
        pts_sample_feat=pts_sample_feat.flatten(3,4).mean(-1).permute(0,2,1) # B,C,Q,G*P -> B,C,Q -> B,Q,C

        fusion_feat=img_sample_feat+pts_sample_feat
