...
class OPUSTransformer_PT_GroupMixing_2D(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=6,
...
class OPUSTransformerDecoderLayer_PT(BaseModule):
    def __init__(self,
        ...
        self.pts_mixing=AdaptiveMixing(in_dim=embed_dims, in_points=num_points ,
                                       n_groups=num_groups)
    ...
    def forward(self, query_points, query_feat, mlvl_feats, pts_feats,occ2img, img_metas):
        ...
        # F.grid_sample(pts_feats, sampling_pts, align_corners=False)

        ## B,Q,C
        img_sample_feat=self.mixing(sampled_feat, query_feat) # sampled_feat: B,Q,G,TP,C; query_feat: B,Q,C
        sampling_pts=encode_points(sampling_pts, self.pc_range)
        sampling_pts=sampling_pts * 2 - 1 # B,Q,G,P,3

        B,Q,G,P,_=sampling_pts.shape
        B,C,Dz,Dy,Dx=pts_feats.shape
        C1=int(C//G)

        pts_feats=pts_feats.reshape(B,G,C1,Dz,Dy,Dx).reshape(-1,C1,Dz,Dy,Dx) # B,G,C1,Dz,Dy,Dx -> B*G,C1,Dz,Dy,Dx
        sampling_pts=sampling_pts.permute(0,2,1,3,4).reshape(B*G,Q,P,3).unsqueeze(2) # B,Q,G,P,3 -> B*G,Q,P,3 -> B*G,Q,P,1,3
        sampling_pts=sampling_pts[...,:2]

        # B*G,C1,Q,1,P
        pts_sample_feat=F.grid_sample(pts_feats, sampling_pts, padding_mode='zeros', align_corners=True) 
        pts_sample_feat=pts_sample_feat.squeeze(-2).reshape(B,G,C1,Q,P).permute(0,3,1,4,2) # B,G,C1,Q,P -> B,Q,G,P,C1
        pts_sample_feat=self.pts_mixing(pts_sample_feat,query_feat) # B,Q,G,P,C1 -> B,Q,C

        fusion_feat=torch.cat([img_sample_feat,pts_sample_feat],dim=2)
        fusion_feat=self.proj_feat(fusion_feat)
