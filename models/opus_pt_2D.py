...
class OPUS_PT_2D(MVXTwoStageDetector):
    '''
    ...
    def __init__(self,
        ...
        last_pts_feat_dim=pts_feat_dim
        for i in range(self.upsample_stages):
            self.predicter_list.append(nn.Sequential(
                nn.Linear(last_pts_feat_dim, last_pts_feat_dim * up_sample),
                nn.Softplus(),
                nn.Linear(last_pts_feat_dim * up_sample, last_pts_feat_dim * up_sample),
            ))
            last_pts_feat_dim *= 2
        assert last_pts_feat_dim==pts_feat_dim*Dz, 'the last pts feat dim is not correct'
        self.Dz=Dz
    ...
    def pts_feat_proj(self,pts_feat):
        ...
        pts_feat = pts_feat.view(bs, Dx, Dy, self.Dz, self.pts_feat_dim)
        pts_feat = pts_feat.permute(0, 4, 3, 2, 1).contiguous() # (B, Dx, Dy, Dz, n_cls) -> (B, n_cls, Dz, Dx, Dy)
