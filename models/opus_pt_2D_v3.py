...
class OPUS_PT_2D_v3(MVXTwoStageDetector):
    '''
        specifically for 2D SECOND

        Adding:
            flashocc style design: let feat -> feat,Dz,256
    ...
    def __init__(self,
        ...
        self.predicter_list=nn.ModuleList()
        for i in range(self.upsample_stages):
            self.predicter_list.append(nn.Sequential(
                nn.Linear(pts_feat_dim, pts_feat_dim * up_sample),
                nn.Softplus(),
                nn.Linear(pts_feat_dim * up_sample, pts_feat_dim * up_sample),
            ))
    ...
    def pts_feat_proj(self,pts_feat):
        ...
        pts_feat=self.final_conv(pts_feat).permute(0, 3, 2, 1)
        bs, Dx, Dy = pts_feat.shape[:3]

        # (B, Dx, Dy, C) --> (B, Dx, Dy,1,C) --> (B, Dx, Dy, Dz, C)
        pts_feat=pts_feat.unsqueeze(-2)
        for predicter in self.predicter_list:
            pts_feat = predicter(pts_feat)
            ori_dim=pts_feat.shape[-2]
            pts_feat =pts_feat.reshape(bs, Dx, Dy, ori_dim, self.upsample, self.pts_feat_dim)
            pts_feat = pts_feat.reshape(bs, Dx, Dy, -1, self.pts_feat_dim)

        assert pts_feat.shape[-2]==self.Dz, 'the shape of pts feat is not correct in pts_feat_proj'
        pts_feat = pts_feat.permute(0, 4, 3, 2, 1).contiguous() # (B, Dx, Dy, Dz, n_cls) -> (B, n_cls, Dz, Dy, Dx)
